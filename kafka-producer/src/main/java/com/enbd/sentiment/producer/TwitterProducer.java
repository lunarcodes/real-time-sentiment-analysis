...
public class TwitterProducer {
    ...
    // Country distribution: UAE 70%, KSA 15%, EGP 5%, SGP 4%, LON 3%, IND 3%
    private static final List<Location> UAE_LOCATIONS = Arrays.asList(
        ...
        new Location("Ajman", "United Arab Emirates", "AE", 25.4052, 55.5136, "Asia/Dubai"),
        new Location("Ras Al Khaimah", "United Arab Emirates", "AE", 25.7895, 55.9432, "Asia/Dubai")
    );
    
    private static final List<Location> KSA_LOCATIONS = Arrays.asList(
        new Location("Riyadh", "Saudi Arabia", "SA", 24.7136, 46.6753, "Asia/Riyadh"),
        new Location("Jeddah", "Saudi Arabia", "SA", 21.4858, 39.1925, "Asia/Riyadh"),
        new Location("Dammam", "Saudi Arabia", "SA", 26.4207, 50.0888, "Asia/Riyadh")
    );
    
    private static final List<Location> EGP_LOCATIONS = Arrays.asList(
        new Location("Cairo", "Egypt", "EG", 30.0444, 31.2357, "Africa/Cairo"),
        new Location("Alexandria", "Egypt", "EG", 31.2001, 29.9187, "Africa/Cairo")
    );
    
    private static final List<Location> SGP_LOCATIONS = Arrays.asList(
        new Location("Singapore", "Singapore", "SG", 1.3521, 103.8198, "Asia/Singapore")
    );
    
    private static final List<Location> LON_LOCATIONS = Arrays.asList(
        new Location("London", "United Kingdom", "GB", 51.5074, -0.1278, "Europe/London"),
        new Location("Manchester", "United Kingdom", "GB", 53.4808, -2.2426, "Europe/London")
    );
    
    private static final List<Location> IND_LOCATIONS = Arrays.asList(
        new Location("Mumbai", "India", "IN", 19.0760, 72.8777, "Asia/Kolkata"),
        new Location("Delhi", "India", "IN", 28.7041, 77.1025, "Asia/Kolkata"),
        new Location("Bangalore", "India", "IN", 12.9716, 77.5946, "Asia/Kolkata")
    );
    
    private static final List<MessageTemplate> POSITIVE_MESSAGES = Arrays.asList(
        new MessageTemplate("The new mobile app is amazing! So easy to use ðŸŽ‰", "mobile_app", Arrays.asList("mobile_app"), 0.85, 0.95),
        ...
        new MessageTemplate("Investment platform is excellent! Great returns ðŸ“ˆ", "investments", Arrays.asList("investments", "platform"), 0.88, 0.93),
        new MessageTemplate("Finally a bank that understands customer needs â¤ï¸", "general", Arrays.asList("customer_experience"), 0.85, 0.91),
    ...
    private static final List<MessageTemplate> NEGATIVE_MESSAGES = Arrays.asList(
        new MessageTemplate("Mobile app keeps crashing! This is unacceptable ðŸ˜¡", "mobile_app", Arrays.asList("mobile_app", "technical_issue"), -0.85, 0.94),
        ...
        new MessageTemplate("Credit card application rejected without explanation ðŸ˜ ", "credit_cards", Arrays.asList("credit_cards", "application"), -0.75, 0.87),
    ...
    private static RawTweet generateRealisticTweet() {
        ...
        // Sentiment distribution: 80% positive, 10% negative, 10% neutral (for positive customer experience)
        double sentimentRoll = rand.nextDouble();
        MessageTemplate template;
        if (sentimentRoll < 0.80) {
            template = POSITIVE_MESSAGES.get(rand.nextInt(POSITIVE_MESSAGES.size()));
        } else if (sentimentRoll < 0.90) {
        ...
        // Country distribution: UAE 70%, KSA 15%, EGP 5%, SGP 4%, LON 3%, IND 3%
        Location location = selectWeightedLocation(rand);
    ...
    /**
     * Select location based on weighted country distribution:
     * UAE: 70%, KSA: 15%, EGP: 5%, SGP: 4%, LON: 3%, IND: 3%
     */
    private static Location selectWeightedLocation(Random rand) {
        double countryRoll = rand.nextDouble();
        List<Location> selectedCountryLocations;
        
        if (countryRoll < 0.70) {
            // UAE - 70%
            selectedCountryLocations = UAE_LOCATIONS;
        } else if (countryRoll < 0.85) {
            // KSA - 15%
            selectedCountryLocations = KSA_LOCATIONS;
        } else if (countryRoll < 0.90) {
            // EGP - 5%
            selectedCountryLocations = EGP_LOCATIONS;
        } else if (countryRoll < 0.94) {
            // SGP - 4%
            selectedCountryLocations = SGP_LOCATIONS;
        } else if (countryRoll < 0.97) {
            // LON - 3%
            selectedCountryLocations = LON_LOCATIONS;
        } else {
            // IND - 3%
            selectedCountryLocations = IND_LOCATIONS;
        }
        
        return selectedCountryLocations.get(rand.nextInt(selectedCountryLocations.size()));
    }
